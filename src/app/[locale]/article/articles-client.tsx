...
export default function ArticlesClient({
  ...
  const tagCounts = useMemo(
    () =>
      articles.reduce<Map<string, number>>((acc, article) => {
    ...
    () =>
      Array.from(tagCounts.entries())
        ...
        .sort((a, b) => {
          ...
          return a.tag.localeCompare(b.tag, locale === "ru" ? "ru" : "en", { sensitivity: "base" });
        }),
    [tagCounts, locale],
  );
  ...
  const filteredArticles = useMemo(
    () =>
      selectedTag
        ? articles.filter((article) => article.tagsList.includes(selectedTag))
        : articles,
        ...
        {filteredArticles.map((article) => (
          <li key={article.slug}>
            <Link
              ...
              className="group block rounded-3xl bg-black/40 p-6 shadow-[0_0_40px_rgba(16,185,129,0.05)] transition hover:border-emerald-400/40 hover:shadow-[0_0_40px_rgba(16,185,129,0.2)]"
            >
